...
export default function DrawToolbar({
  ...
  return (
    <div className="toolbar draw-mode" role="toolbar" aria-label="Drawing">
      ...
      <div className="tb-group">
        {WIDTHS.map((w) => (
          <button
            ...
            className={`tb-btn ${width === w ? "is-active" : ""}`}
            onClick={() => setWidth(w)}
          >
      ...
      <div className="tb-group">
        <button type="button" className="tb-btn" title="Delete selected" aria-label="Delete selected"
          onClick={onDeleteSelected} disabled={!hasSelection}>
          <Trash2 size={17} />
        </button>
        <button type="button" className="tb-btn" title="Clear all drawing" aria-label="Clear all drawing" onClick={onClear}>
          <Eraser size={17} />
        </button>
      </div>

      <button type="button" className="tb-btn draw-done" onClick={onDone} style={{ marginLeft: "auto" }}>
        <Check size={16} /> Done
      </button>
    </div>
  );
